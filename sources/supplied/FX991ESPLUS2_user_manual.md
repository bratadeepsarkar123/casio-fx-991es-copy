# fx-115ES PLUS / fx-991ES PLUS C User’s Guide (2nd edition / NATURAL-V.P.A.M.)

Evidence class: `CROSS-MODEL-SOURCE`. Not `TARGET-MANUAL` for FX-991ESPLUS-2.
Source PDF: `sources/supplied/FX991ESPLUS2_user_manual.pdf`
...
fx-115ES PLUS/fx-991ES PLUS C: Every 3 years
...
Pressing the
  or
  key followed by a second key performs the
alternate function of the second key
...
operation (
 ) before it. Note that this is not part of the actual key
...
(3) Indicators
• If a
 indicator appears on the right side of the calculation result, it
means the displayed calculation result continues to the right. Use
and
  to scroll the calculation result display.
• If a
  indicator appears on the right side of the input expression, it
...
input expression while both the
 and
  indicators are displayed, you
will need to press
  first and then use
  and
  to scroll.
...
• The
 indicator in the upper right corner of a menu means there is
another menu below the current one. The
  indicator means another
menu above. Use
  and
...
To specify this
display format: Perform this key operation:
Natural Display
(MthIO-MathO)
(SETUP)
 (MthIO)
...
default angle unit: Perform this key operation:
Degrees
 (SETUP)
 (Deg)
Radians
...
f(x) and g(x)
 ...
 (TABLE)
 (f(x),g(x))
Specifying the Recurring Decimal Format
Specifies whether or not to display calculation results using recurring
decimal form.
To specify this: Perform this key operation:
...
and certain functions (log,
 ...
 ,
 , ∫, d/dx, Σ,
Π, Abs) just as they are written in your textbook.
Example: 2 + √2
1 + √2  (MthIO-MathO)
2
 2
...
expression you input may be cut of
f. If you need to view the entire input expression
again, press
  and then use
  and
...
When a calculation result takes the form ± a√b
c  ± d√e
f
*
, √  form calculation
...
Use
  and
  to move the cursor to the location where you want to
...
• With certain calculation results, pressing the
  key will not convert the displayed
value.
• When ON is selected for Rdec on the setup menu, pressing
  will switch the
calculation result to recurring decimal form. For details, see "Recurring Decimal
Calculations".
...
pressing
  instead of
  will display the calculation result in decimal form: a
calculation that results in a
√  form or π form expression, a division calculation.
Pressing
...
Multi-Statements
Y
ou can use the colon character (:) to connect two or more expressions
...
1.234×103
1234×100
Example 2: T ransform the value 123 to engineering notation, shifting the
...
Y
our calculator uses a recurring decimal when you input a value.
...
period (repetend) and then input the period up to the ending value. T
o
...
3
Example 2: T o input 1.428571428571... (1.428571)  (MthIO-MathO)
1
 (
 )
26
...
```text
428571
Example 3: T o calculate 1.021 + 2.312
1
 (
 ) 021
2
...
Calculation result displayed as recurring decimal
value:
Note
• You can specify up to 14 decimal places for the recurring decimal period. If you input
more than 14 decimal places, the value will be treated as a terminating decimal and
not a recurring decimal.
• Recurring decimal value input can be performed regardless of the Rdec setting on the
setup menu.
Displaying a Calculation Result as a Recurring Decimal V
alue
Calculation results that can be displayed as recurring decimal values will
be displayed as such when ON is selected for the Rdec setting on the
setup menu.
Pressing the
 key will cycle between the available calculation result
formats.
Example 1: 1
7 = 0.142857 = 0.1428571429 (Norm 1)  (MthIO-MathO)
1
 7
...
Example 2: 1 ÷ 7 = 1
7 = 0.142857 = 0.1428571429 (Norm 1)  (MthIO-
MathO)
1
 7
Display as fraction:
...
Example 3: 1
7 = 0.142857 = 0.1428571429 (Norm 1)  (LineIO)
1
 7
Display as recurring decimal:
Decimal value according to Norm 1 setting:
...
Example 3: T o confirm the following: 0.123 = 123
999, 0.1234 = 1234
9999, 0.
12345
= 12345
...
(Continuing)  789
Using PreAns Memory
Example: For Tk+2 = Tk+1 + Tk (Fibonacci sequence), determine the
sequence from T1 to T5. Note however, that T1 = 1 and T2 = 1.
T1 = 1
1
(Ans = T1 = 1)
T2 = 1
...
Y
our calculator has nine preset variables named A, B, C, D, E, F, M, X,
and Y.
...
PreAns memory contents are retained even if you press
  and turn
of
f the calculator without exiting the COMP Mode.
...
waiting for the calculation result to appear
...
e is displayed as 2.718281828, but e
...
2
 ...
 (x3)
 8
Example 5: T o calculate ln 90 (= loge 90) to three significant digits  (Sci 3)
(SETUP)
 (Sci)
90
 4.50×100
...
(LineIO)  5
 ...
 2
Example 5: T o calculate √2 × 3 (= 3√2 = 4.242640687...) to three decimal
places  (Fix 3)
(SETUP)
 (Fix)
38
...
∫b
a f (x)dx, while Linear Display input syntax
is ∫ (f (x), a, b, tol).
tol specifies tolerance, which becomes 1 × 10-5 when nothing is input for
tol.
Example 1: ∫e
1 ln(x) = 1
(MthIO-MathO)
...
dif
ference method.
Natural Display input syntax is d
dx (f (x)) | x=a, while Linear Display input
syntax is d
dx (f (x), a, tol).
tol specifies tolerance, which becomes 1 × 10-10 when nothing is input for
tol.
Example 1: T
o obtain the derivative at point x = π/2 for the function y =
...
(π)
 ...
 0
Example 2: d
dx (3x2 - 5x + 2, 2, 1 × 10-12) = 7  (LineIO)
41
```
...
∏ Calculations
Y
ou can obtain the product of an input f(x) expression for a specific range.
The calculation formula is:
 (f(x)) = f(a) × f(a + 1) × f(a + 2) × ⋯ × f(b).
The Natural Display input syntax is
 (f(x)), while Linear Display input
syntax is ∏(f(x), a, b).
a
and b are integers in the range of a < 1 × 1010, b < 1 × 10 10, a ≦ b.
Example:
 (x + 1) = 720
(MthIO-MathO)
(
...
(Pol)
 ...
 r = 2
θ = 45
Example 2: T o convert polar coordinates (√2, 45°) to rectangular
coordinates  (Angle unit: Deg)
(MthIO-MathO)
(Rec)
 2
...
```text
Example: T o determine the greatest common divisor of 28 and 35
(GCD) 28
 (,) 35
 7
Least Common Multiple (LCM)
...
Example: T
o determine the least common multiple of 9 and 15
(LCM) 9
 (,) 15
 45
Integer Part of a Value (Int) and
Largest Integer that does not Exceed
a V
alue (Intg)
Integer Part of a Value (Int)
Extracts the integer part of a value.
Example: To extract the integer part of -3.5
(Int)
 3
 5
 -3
Largest Integer that does not Exceed a Value (Intg)
Determines the largest integer that does not exceed a value.
Example: T
o determine the largest integer that does not exceed -3.5
(Intg)
 3
 5
 -4
...
T
o start a CALC operation after inputting an expression, press the
key.
Example 1: T
o store 3A + B and then substitute the following values to
...
7
 20
To exit CALC:
Example 2: T o store A+Bi and then determine √3 + i, 1 + √3i using polar
...
• Equations that include variable X: X2 + 2X - 2, Y = X + 5, X = sin(M),
X + 3 = B + C
SOL
VE solves for X. An expression like X2 + 2X - 2 is treated as X2 + 2X
- 2 = 0.
• Equations input using the following syntax: {equation}, {solution
variable}
SOL
VE solves for Y, for example, when an equation is input as: Y = X +
5, Y
Important!
...
• The following functions are not allowed inside of an equation: ∫, d/dx, Σ, Π, Pol, Rec,
÷R.
Example: T
o solve y = ax2 + b for x when y = 0, a = 1, and b = -2
(Y)
 (=)
(A)
 (X)
...
equations like the following: y = sin(x), y = ex, y = √
x.
...
Example: T
o solve y = x2 - x + 1 for x when y = 3, 7, and 13.
(Y)
 (=)
(X)
 (X)
...
want.
Example 1: T
o input the scientific constant C0 (speed of light in a
vacuum), and display its value
(CONST)
(C0)
Example 2: T o calculate C0 = 1
√ε0μ0
  (MthIO-MathO)
1
 (CONST)
...
constant, rationalized 10: (α) fine-structure constant
...
19: (μe) electron magnetic
moment 20: (μn) neutron magnetic moment
...
gravity 36: (G0) conductance quantum
37: (Z0) characteristic impedance
of vacuum 38: (t) Celsius temperature
...
command you want.
Example 1: T
o convert 5 cm into inches  (LineIO)
5
 (CONV)
(cm
 in)
Example 2: T o convert 100 g into ounces  (LineIO)
100
 (CONV)
 (g
 oz)
Example 3: T o convert -31°C into Fahrenheit  (LineIO)
31
 (CONV)
 (°C
 °F)
...
(CMPLX)
T
o perform complex number calculations, first press
 (CMPLX) to
...
Example 1: (1 - i)-1 = 1
2 + 1
2i  (MthIO-MathO) (Complex number format: a
+bi)
1
 (i)
 1
...
1
 ...
 (i)
 0
Example 3: T o obtain the conjugate complex number of 2 + 3i
(Complex number format: a+bi)
(CMPLX)
 (Conjg) 2
 3
 (i)
 2-3i
Example 4: T o obtain the absolute value and argument of 1 + i  (MthIO-
...
Example: √2 + √2i = 2∠45, 2∠45 = √2 + √2i  (MthIO-MathO) (Angle unit:
Deg)
2
 2
 (i)
...
T
o start a statistical calculation, perform the key operation
 (STAT)
to enter the ST
...
```text
The Statistics Editor provides 40 rows for data input when there is an X
column only or when there are X and Y columns, 20 rows when there are
X and FREQ columns, or 26 rows when there are X, Y
, and FREQ
columns.
Note
• Use the FREQ (frequency) column to input the quantity (frequency) of identical data
items. Display of the FREQ column can be turned on (displayed) or of
f (not displayed)
...
• The following operations are not supported by the Statistics Editor:
 ,
(M-),
 (STO). Pol, Rec, ÷R, and multi-statements also cannot be input with
...
with the data you input with the Statistics Editor
. Pressing the
  key
while the Statistics Editor is displayed switches to the Statistics Calculation
Screen.
Using the Statistics Menu
While the Statistics Calculation Screen is on the display
, press
(STAT/DIST) to display the Statistics Menu.
...
```text
(Var) Display the Var sub-menu of commands for
...
(MinMax) Display the MinMax sub-menu of commands
...
Commands when Quadratic Regression Calculation (_+CX2) Is
Selected
Reg Sub-menu (
 (STAT/DIST)
 (Reg))
...
item: When you want to obtain this:
(A) Regression coefficient constant term A
(B) Linear coefficient B of the regression
coef
ficients
(C) Quadratic coefficient C of the regression
coef
ficients
...
(yˆ) Estimated value of y
Note
•  xˆ, xˆ1, xˆ2 and yˆ are not variables. They are commands of the type that take an
argument immediately before them. See "Calculating Estimated V
alues" for more
information.
...
items ({xn; freqn} = {1;1, 2;2, 3;3, 4;2, 5;1}), and calculate the mean and
...
octal.
Example 1: T
o enter the BASE-N Mode, switch to the binary mode, and
calculate 112 + 12
(BASE-N)
(BIN)
11
...
supported. If a calculation result has a fractional part, it is cut of
f.
...
Binary
0000000000000000 ≦ x ≦ 01 11111111111111
1000000000000000 ≦ x ≦ 1 111111111111111
Octal
...
(decimal),
 ...
 (OCT) (octal).
Example: T
o calculate 1510 × 3710 in the decimal mode, and then convert
the result to hexadecimal, binary
, and octal
(DEC) 15
 37
 555
...
Y
our calculator provides you with logical operators (and, or, xor, xnor) and
...
(and) Logical operator "and" (logical product),
which returns the result of a bitwise AND
(or) Logical operator "or" (logical sum), which
...
(Not) "Not(" function, which returns the result of
a bitwise complement
(Neg) "Neg(" function, which returns the result of
a two's complement
All of the following examples are performed in the binary mode (
(BIN)).
Example 1: T
o determine the logical AND of 10102 and 11002  (10102 and
1
1002)
1010
 (BASE)
 (and) 1100
 0000000000001000
Example 2: T o determine the logical OR of 10112 and 110102  (10112 or
1
10102)
1011
 (BASE)
 (or) 11010
 0000000000011011
Example 3: T o determine the logical XOR of 10102 and 11002  (10102 xor
1
1002)
1010
 (BASE)
...
• To solve 2x2 + x - 3 = 0, for example, press
  in step 2, and then
...
• The following operations are not supported by the Coefficient Editor:
 ,
(M-),
 (STO). Pol, Rec, ÷R, and multi-statements also cannot be
input with the Coef
ficient Editor.
...
infinite solutions. Pressing
 or
  will return to the Coefficient Editor.
Changing the Current Equation T
ype Setting
Press
 (EQN) and then select an equation type from the menu that
appears. Changing the equation type causes the values of all Coef
ficient
...
(Y=)    2
(Z=)    3
Example 3: 2x2 - 3x - 6 = 0  (MthIO-MathO)
(EQN)
 (aX2 + bX + c = 0)
2
 3
...
Whenever the result of a calculation executed in the MA
TRIX Mode is a
...
,
 ...
 ,
 ,
 (x3). The calculation screen will show the
...
• The following operations are not supported by the Matrix Editor:
 ,
 (M-),
(STO). Pol, Rec, ÷R, and multi-statements also cannot be input with the
Matrix Editor
.
To assign new data to a matrix variable:
1. Press
 (MATRIX)
 (Dim), and then, on the menu that
...
1. Press
 (MATRIX)
 (Data), and then, on the menu that
...
.
T
o copy matrix variable (or MatAns) contents:
...
the MatAns screen:
 ...
 .
2. Press
 (STO), and then perform one of the following key
operations to specify the copy destination:
 (MatA),
 (MatB), or
(MatC).
...
(MATRIX)
 (MatA)
 1
Example 5: Obtain the transposition of MatC (T rn(MatC)).
75
```
...
(MATRIX)
 (MatC)
Example 6: Obtain the inverse matrix of MatA (MatA-1).
Note: Y
ou cannot use
...
(x3) to specify cubing.
(MATRIX)
 (MatA)
(MATRIX)
 (MatA)
 (x3)
Example 9: Determine the MatA =
  row echelon form.
(MATRIX)
 (Ref)
(MATRIX)
 (MatA)
Example 10: Determine the MatA =
...
3. In response to the prompts that appear
, input the values you want to
use, pressing
  after each one.
...
• Pressing
  while the numerical table screen is displayed will return
...
```text
Example: T o generate a numerical table for the functions f(x) = x2 + 1
2
and g(x) = x2 - 1
2 for the range -1 ≦ x ≦ 1, incremented in steps of
0.5  (MthIO-MathO)
(TABLE)
(SETUP)
 (TABLE)
...
(X)
 ...
 2
・Pressing
  without inputting anything for g(x) will generate a
...
display the setup menu in the T
ABLE Mode and switch between Natural Display
and Linear Display.
78
```
...
Use the VECT
OR Mode to perform 2-dimensional and 3-dimensional
...
for VctA.
(1) "A" stands for "VctA".
3. Input the elements of VctA: 1
 2
 .
4. Perform the following key operation:
(VECTOR)
...
• This will display the Vector Editor for input of the 2-dimensional vector
for VctB.
5. Input the elements of VctB: 3
4
 .
6. Press
  to advance to the calculation screen, and perform the
...
Note: "VctAns" stands for "V ector Answer Memory". See "Vector
...
Whenever the result of a calculation executed in the VECT
OR Mode is a
...
2. Use the V
ector Editor that appears to edit the elements of the vector.
• Move the cursor to the cell that contains the element you want to
change, input the new value, and then press
 .
T
o copy vector variable (or VctAns) contents:
...
Example 3: 3 × VctA (Vector scalar multiplication), 3 × VctA - VctB
...
(VECTOR)
 (VctB)
Example 4: VctA • VctB (V ector dot product)
(VECTOR)
 (VctA)
(VECTOR)
 (Dot)
(VECTOR)
 (VctB)
Example 5: VctA × VctB (V ector cross product)
(VECTOR)
 (VctA)
(VECTOR)
 (VctB)
...
Y
ou can use the following procedure to solve a quadratic inequality or
cubic inequality.
1. Press
 (INEQ) to enter the INEQ Mode.
2. On the menu that appears, select an inequality type.
To select this inequality
type: Press this key:
...
• To solve x2 + 2x - 3 < 0, for example, input the coef ficients a = 1, b = 2,
c = -3 by pressing 1
 2
 3
 .
...
1
 2
 3
Example 2: x2 + 2x - 3 ≧ 0   (MthIO-MathO)
(INEQ)
 (aX2 + bX + c)
(aX2 + bX + c ≧ 0)
1
...
2
 3
Example 4: 3x3 + 3x2 - x > 0   (MthIO-MathO)
(INEQ)
 (aX3 + bX2 + cX + d)
(aX3 + bX2 + cX + d > 0)
3
...
< 1 + 1, 3 < π < 4, 22 = 2 + 2 = 4 < 6, 2 + 3 = 5 ≠ 2 + 5 = 8, etc.
Note
...
Y
ou can use the procedures below to perform seven different types of
distribution calculations.
1. Press
...
• With Binomial PD, Binomial CD, Poisson PD, and Poisson CD, you
can input sample data and then perform calculations.
4. After inputting values for all of the variables, press
 .
• This displays the calculation results.
• Pressing
  or
  while a calculation result is displayed will return to
...
```text
To edit sample data:
Move the cursor to the cell that contains the data you want to edit, input
the new data, and then press
 .
T
o delete data:
...
Move the cursor to the position where you want to insert the sample data,
press
 (STAT/DIST)
 (Edit)
 (Ins), and then input the sample
data.
T
o delete all sample data:
Press
 (STAT/DIST)
 (Edit)
 (Del-A).
...
Example 1: T
o calculate the normal probability density when x = 36, σ = 2,
μ = 35
(DIST)
(Normal PD)
36
...
Display the List Screen:
 (List)
• To specify data using parameter format, press
 (Var).
10
 11
 12
...
0
 6
x = binomial probability of 10 ≒ 0.18594
x = binomial probability of 1
1 ≒ 0.12678
x = binomial probability of 12 ≒ 0.063388
x = binomial probability of 13 ≒ 0.021942
x = binomial probability of 14 ≒ 4.7018 × 10-3
• Pressing
  returns to the N input screen. Pressing
  returns to the
List Screen (input data samples are stored).
Note
• The following cannot be used in the distribution calculations: Pol, Rec, ÷R, ∫, d/dx.
• When data is specified using parameter format, calculation results are stored in Ans
memory
.
90
```
...
```text
• An error message appears if the input value is outside the allowable range. "ERROR"
will appear in the Ans column of the List Screen when the value input for the
corresponding sample data is outside the allowable range.
91
```
...
V
ariable ERROR (SOLVE feature only)
Cause:
...
A low battery is indicated by a dim display
...
2 Functions that require an argument to the right and a closing
parenthesis ")" following the argument
3 Functions that come after the input value (x2, x3, x-1, x!, °’ ”, °,
r, g, %,
t), powers (
 ), roots (
 )
...
6 Metric conversion commands (cm
 in, etc.), STAT Mode
estimated values (xˆ, yˆ, xˆ1, xˆ2)
7 Multiplication where the multiplication sign is omitted
8 Permutation (nPr), combination (nCr), complex number polar
coordinate symbol (∠)
9 Dot product (•)
10 Multiplication (×), division (÷), remainder calculations (÷R)
...
enclosed in parentheses (
 2
 ). Since x2 has a higher priority than
...
Gra 0 ≦ |x| < 1 × 1010
tanx
Deg Same as sinx, except when |x| = (2n-1) ×
90.
Rad Same as sinx, except when |x| = (2n-1) ×
π/2.
...
```text
RanInt#(a, b) a < b; |a|, |b| < 1 × 1010; b - a < 1 × 10 10
