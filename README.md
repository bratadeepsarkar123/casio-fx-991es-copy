...
P0 COMP-mode behavior is covered by golden tests. TABLE f(x) (D-016) and BASE-N (D-017) are implemented. Other P1 modes (STAT, EQN, MATRIX, VECTOR, CMPLX) can be **entered** from MODE but are not verified editors. Functional-v1 vs complete-clone counts are separate; see `CAPABILITY_MATRIX.md` and `docs/COVERAGE_AUDIT.md`.
